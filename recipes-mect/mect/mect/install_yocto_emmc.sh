...
then
	...
	echo "Ex: install_yocto_emmc.sh tp2101-01-a" 
        exit 0
else
	Machine_name=$1
...
function install_kernel
{
	...
	cp -v ${imagesdir}/${Machine_name}.dtb		${mountdir_prefix}${bootpart}
	cp -v ${imagesdir}/uImage			${mountdir_prefix}${bootpart}
	sync
	echo "Setting U-Boot enviroment variables"
	set_fw_utils_to_emmc_on_sd_card
	fw_setenv fdt_file ${Machine_name}.dtb
...
/etc/X11/Xsession.d/73touchscreen-settings.sh
exit 0
